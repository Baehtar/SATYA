...
const FLAG_LABELS = {
    ...
    RECYCLED_IMAGE: 'Recycled / out-of-context image',
    PUBLISHED_ONLINE_BEFORE: 'Published online before',
    MANIPULATION_SIGNALS: 'Manipulation signals present',
    CLONED_REGION: 'Duplicated region in the image',
};

// Reverse-search outcomes. The wording matters: "no copies located" is not
// "original", and "could not check" is not "nothing found".
const PROVENANCE_LABELS = {
    RECYCLED: ['🔁', 'Recycled / out of context', 'bad'],
    CONTEMPORANEOUS: ['🗓️', 'Consistent with the claimed date', 'ok'],
    PREVIOUSLY_PUBLISHED: ['🌐', 'Published online before', 'warn'],
    SIMILAR_ONLY: ['🔎', 'Only similar images found', 'neutral'],
    NO_MATCHES_LOCATED: ['🔎', 'No earlier copies located', 'neutral'],
    SEARCH_UNAVAILABLE: ['⚠️', 'Provenance check unavailable', 'neutral'],
};
...
export function renderVerdict(data) {
    ...
    // ── Image provenance ─────────────────────────────────────────────────────
    // Rendered as its own section, never folded into the claim verdict: where a
    // photo has been before is a different question from whether the sentence
    // about it is true. Shown in every mode, including ai_image.
...
function renderProvenance(provenance) {
    const section = document.getElementById('provenance-section');
    const statusEl = document.getElementById('provenance-status');
    const factsEl = document.getElementById('provenance-facts');
    const matchesEl = document.getElementById('provenance-matches');
    const noteEl = document.getElementById('provenance-note');
    ...
    if (!provenance || !provenance.status) {
        section.classList.add('hidden');
        return;
    }
    section.classList.remove('hidden');

    const [icon, label, tone] = PROVENANCE_LABELS[provenance.status] ||
        ['🔎', provenance.status, 'neutral'];
    statusEl.className = `provenance-status ${tone}`;
    statusEl.textContent = `${icon} ${label}`;

    const facts = [];
    const earliest = formatDate(provenance.earliest_located_date);
    if (earliest) {
        // Deliberately "located", not "original" — we only see indexed copies.
        facts.push(['Earliest located appearance', earliest]);
    }
    if (provenance.claim_date) {
        facts.push(['Date claimed', formatDate(provenance.claim_date)]);
    }
    const gap = provenance.date_difference_days;
    if (gap && gap > 30) {
        const years = gap / 365.25;
        facts.push(['Difference', years >= 1 ? `~${years.toFixed(1)} years` : `${gap} days`]);
    }
    if (provenance.n_matches) {
        facts.push(['Copies found online', String(provenance.n_matches)]);
    }
    ...
    (provenance.matches || []).slice(0, 4).forEach(match => {
        if (!match.url) return;
        const a = document.createElement('a');
        a.href = match.url;
        a.className = 'source-pill';
        a.target = '_blank';
        a.rel = 'noopener noreferrer';
        const when = formatDate(match.published_date);
        a.textContent = when ? `${match.domain} · ${when}` : match.domain;
        a.title = match.title || match.url;
        matchesEl.appendChild(a);
    });

    // The caveat is the most important part of this section — never drop it.
    if (provenance.notes && provenance.notes.length > 0) {
        noteEl.textContent = provenance.notes[0];
    }
}
